...
1. Run these migrations in order in the Supabase SQL editor:
   ...
   - `supabase/migrations/20260919_005_harden_room_claim.sql`
...
Room-join reliability checks:

- A malformed room code is rejected before any room row is touched.
- A closed, unknown or already-claimed room is rejected with a student-friendly message.
- A room without exactly one compatible active lesson session is not left claimed: the RPC raises an exception and the transaction rolls the room update back.
...
- Room claiming happens through a security-definer RPC and is atomic across both the room and its active lesson session.
