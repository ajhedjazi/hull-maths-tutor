...
test("fetchClassroomSnapshot returns authoritative room, latest question and answer", async () => {
  const room = { id: "room-1", status: "active" };
  const question = { id: "sq-2", session_id: "session-1", position: 2 };
  const answer = { id: "answer-1", session_question_id: "sq-2", is_correct: true };
  const calls = [];
  const supabase = {
    from(table) {
      calls.push(table);
      ...
      if (table === "student_answers") return query({ data: [answer], error: null });
...
test("fetchClassroomSnapshot skips answer lookup when there is no question", async () => {
  ...
  const supabase = {
    from(table) {
      calls.push(table);
      if (table === "rooms") return query({ data: { id: "room-1" }, error: null });
